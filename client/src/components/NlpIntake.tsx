/** Community Wayfinding: NLP-assisted intake turns a user’s own words into a transparent questionnaire choice, not a diagnosis. */
...
export function NlpIntake({ language, unlocked, onRequestConsent, onUseRoute }: { language: Language; unlocked: boolean; onRequestConsent: () => void; onUseRoute: (route: ScenarioId, concern: string, analysis: IntakeResult) => void }) {
  ...
  const submit = () => {
    ...
    if (concern.trim().length >= 8) classify.mutate({ concern: concern.trim(), language, consentAcknowledged: true });
  };
  const chooseSuggestedRoute = () => {
    if (!analysis || analysis.route === "general") return;
    onUseRoute(analysis.route, concern, analysis);
  };

  return <section className="nlp-intake" aria-labelledby="nlp-intake-title"><div className="nlp-intake-heading"><span className="nlp-symbol"><BrainCircuit /></span><div><p className="eyebrow">AI-assisted intake · non-diagnostic</p><h3 id="nlp-intake-title">Describe your concern in your own words.</h3><p>PxK will suggest a focused questionnaire and show the route logic. It does not determine what medical condition you have.</p></div></div>{!unlocked ? <button className="intake-consent-lock" onClick={onRequestConsent}><LockKeyhole /><span><strong>Start with privacy consent</strong><small>Consent is required before entering a health concern.</small></span><ArrowRight /></button> : <><label className="sr-only" htmlFor="nlp-concern">Describe your concern</label><textarea id="nlp-concern" value={concern} onChange={(event) => setConcern(event.target.value)} placeholder={language === "kn" ? "ನಿಮ್ಮ ತೊಂದರೆಯನ್ನು ನಿಮ್ಮ ಸ್ವಂತ ಪದಗಳಲ್ಲಿ ವಿವರಿಸಿ…" : "For example: I have had a cough and feel short of breath after walking…"} maxLength={1200} className="nlp-textarea" /><div className="nlp-actions"><span>{concern.length}/1200 · {isAuthenticated ? "Saved intake available" : "Sign in to analyse and save"}</span><button className="btn btn-primary" disabled={concern.trim().length < 8 || classify.isPending || loading} onClick={submit}>{classify.isPending ? "Finding the questionnaire…" : !isAuthenticated ? "Sign in to continue" : "Find my questionnaire"}<ArrowRight className="h-4 w-4" /></button></div>{classify.error && <p className="nlp-error">The assisted route is unavailable right now. You can still choose a questionnaire manually.</p>}{analysis && <div className="nlp-result"><div><p className="eyebrow">Suggested route</p><h4>{analysis.route === "general" ? "Choose a route manually" : `${analysis.route.charAt(0).toUpperCase()}${analysis.route.slice(1)} questionnaire`}</h4><p>{analysis.summary}</p></div><span className={`nlp-confidence nlp-confidence--${analysis.confidence}`}>{analysis.confidence} signal</span>{analysis.matchedTerms.length > 0 && <div className="matched-terms">{analysis.matchedTerms.map((term) => <span key={term}><CheckCircle2 />{term}</span>)}</div>}<p className="nlp-safety"><Sparkles />{analysis.safetyNotice}</p>{analysis.route !== "general" && <button className="btn btn-primary" onClick={chooseSuggestedRoute}>Use this questionnaire <ArrowRight className="h-4 w-4" /></button>}</div>}</>}</section>;
}
