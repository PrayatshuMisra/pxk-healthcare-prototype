...
export function ScreeningDemo({ language }: { language: Language }) {
  ...
  const payload = (status: "draft" | "completed") => scenarioId ? { publicId: draftId, status, scenarioId, language, concernText: concernText || undefined, answers, currentStep: step, consentVersion: "demo-consent-v2", nlpSummary: intake?.summary, nlpMatchedTerms: intake?.matchedTerms } : null;
  const saveProgress = () => { const input = payload("draft"); if (!input || !isAuthenticated) return; saveDraft.mutate(input); };
